...
def resize_handler(file_path, num_files, x_rez, y_rez):
    ...
    for i in range(4):
        ...
        loadData = LoadData(file_path=file_path,num_files=data_set_files, start_file=start_file)
    ...
    fully_resized_set = np.concatenate((first_data_thread.resized_set,second_data_thread.resized_set,
     third_data_thread.resized_set, fourth_data_thread.resized_set), dtype=np.float32)

    all_y_data = np.concatenate((y_data_sets[0],y_data_sets[1],y_data_sets[2],y_data_sets[3]))

    # Save Resized Data set to a .mat file    
    matfile_name = "./Resized_Sets/" + "Resized_Set" + "_" + str(fully_resized_set.shape) + ".mat"
    save_dict = {"image_patches":fully_resized_set, "category_labels":all_y_data}
    try:
